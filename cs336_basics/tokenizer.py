...
class Tokenizer:
    ...
    def __init__(self, 
        ...
        special_tokens: list[str] | None = None 特殊符号列表
        '''
        self.vocab = vocab
        self.merges = merges
    ...
    def from_files(cls, vocab_filepath: str, merges_filepath: str, special_tokens: list[str]=None):
        """
        从文件加载一个BPE tokenizer。

        Args:
            vocab_filepath: 词汇表的文件路径。
            merges_filepath: 合并规则的文件路径。
            special_tokens: 可选的特殊标记列表，这些标记将直接添加到词汇表中
    ...
    def encode(self, text: str) -> list[int]:
        ...
        for part in pretokenizeiton:
            if self.special_tokens is not None and part in self.special_tokens:
    ...
    def encode_text(self, pre_token: str)-> list[int]:
        '''
        对输入文本（无special_token）进行编码，返回一个包含token id的列表。
    ...
    def apply_merge(self, word_byte: bytes):
        """
        对单词进行合并。
